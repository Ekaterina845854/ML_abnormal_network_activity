...
def capture_traffic(duration=60, output_dir="traffic_logs", file_prefix="traffic_capture"):
    """
    Функция захвата сетевого трафика в течение заданного времени и сохранения в pcap-файл.
    :param duration: Длительность сбора трафика в секундах (по умолчанию 24 часа).
    :param output_dir: Директория для сохранения логов.
    :param file_prefix: Префикс имени файла.
    """
...
def process_pcap(input_file, output_dir="traffic_logs", file_prefix="traffic_data"):
    """
    Функция обработки pcap-файла и сохранения данных в удобном формате (CSV).
    :param input_file: Файл pcap для обработки.
    ...
    df = pd.DataFrame(data, columns=["timestamp", "src_ip", "dst_ip", "length", "protocol"])
...
def extract_features(df):
    ...
    for (src_ip, dst_ip), group in grouped:
        ...
        duration = (last_timestamp - first_timestamp).total_seconds() if pd.notnull(first_timestamp) and pd.notnull(last_timestamp) else 0

        features.append([src_ip, dst_ip, total_packets, total_length, unique_protocols, duration])

    feature_matrix = pd.DataFrame(features, columns=['src_ip', 'dst_ip', 'total_packets', 'total_length', 'unique_protocols', 'duration'])
...
# Функция для запуска непрерывного сбора данных
def continuous_capture(interval=86400, output_dir="traffic_logs"):
    """
    Запускает непрерывный сбор трафика с интервалом сохранения.
    :param interval: Временной интервал (в секундах) для сбора и сохранения данных.
    :param output_dir: Директория для сохранения файлов.
    """
    while True:
        pcap_file = capture_traffic(duration=interval, output_dir=output_dir)
        process_pcap(pcap_file, output_dir=output_dir)
        time.sleep(1)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Network Traffic Capture Script")
